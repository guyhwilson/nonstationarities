...
def traintest_DecoderHMMUnsupervised(struct, decoder, stateTrans, pStateStart, targLocs, vmKappa, 
	...
	#train_x, test_x, train_y, test_y = getTrainTest(struct, train_frac = train_frac, task = task, return_flattened = True)
	train_x, cursorPos, train_y, test_x, _, test_y = prep_HMMData(struct, train_frac = train_frac, task = task, return_flattened = True)

	train_samples                    = train_x.shape[0]

	#cursorPos            = np.vstack([struct.cursorPos[i] for i in np.where(struct.trialType == task)[0] ])[:train_samples, :]
	posTraj              = np.vstack([struct.targetPos[i] for i in np.where(struct.trialType == task)[0] ])[:train_samples, :]
	rawDecTraj           = decoder.predict(train_x - train_x.mean(axis = 0))

	targStates, logP = hmmviterbi_vonmises(rawDecTraj, stateTrans, targLocs, cursorPos, pStateStart, vmKappa)
	pTargState, pSeq = hmmdecode_vonmises(rawDecTraj, stateTrans, targLocs, cursorPos, pStateStart, vmKappa)
	...
	while not isFitted:
		try:
			decoder.fit(train_x[highProbIdx, :] - train_x.mean(axis = 0), inferredPosErr[highProbIdx, :])
			isFitted = True
		except:
			probThreshold -= 0.1
			highProbIdx    = np.where(maxProb > probThreshold)[0]
	...
	# performance stats:
	test_score = decoder.score(test_x - train_x.mean(axis = 0), test_y)

...
class ClickDecoder(object):
	...
	def __init__(self, window, n_components, PCA_window = 20):
		...
		self.PCA_window  = PCA_window                                             # time window around click times to use for PCA subspace ID
	...
	def train(self, features, targets):
		...
		click_times  = click_times[np.logical_and(click_times > self.PCA_window, click_times < features.shape[0] - self.PCA_window)]
		neural_click = np.zeros(((self.PCA_window * 2) + 1, self.n_init_features, len(click_times)))

		for i, t in enumerate(click_times):
			neural_click[:, :, i] = features[(t - self.PCA_window):(t + self.PCA_window + 1), :]

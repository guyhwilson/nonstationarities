def simulateBCIFitts(neuralTuning, D, alpha, beta, nDelaySteps, delT, nStepsForSim):
    
    # First we define the internal state estimator of the BCI user (which we simulate here with a Kalman filter).
    # We begin by defining the decoder dynamics, which are a simple first-order smoother + integrator (damped point mass dynamics). 
    # See 'help ss' for definitions of these variables.
    ...
    D_aug = np.zeros((C_aug.shape[0], B.shape[2]))
    ...
    sysDelay = ss(A_aug, [B_aug G_aug], C_aug, [D_aug H_aug], delT); #IMPLEMENT

    QN = np.eye(4)
    RN = np.eye(4)
    [KEST, L_kalman] = kalman(sysDelay, QN, RN);   # IMPLEMENT
    ...
    for t=1:nStepsForSim
        ...
        distWeight = interp1([0; fTarg(:,1); 100], [fTarg(1,2); fTarg(:,2); fTarg(end,2)], targDist);
